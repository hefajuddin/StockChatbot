...
export default function ChatPage() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const specificResponse = result?.results?.generalResponse?.specificResponse ?? [];
      const responseList = result?.responseList ?? [];

      const botMsg = { sender: "bot", recommendResponse, specificResponse, responseList };
